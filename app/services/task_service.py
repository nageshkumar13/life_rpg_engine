...
class TaskService:
    ...
    def _get_owned_task(self, user_id: str, task_id: str) -> Task:
    ...
    def get_task(self, user_id: str, task_id: str) -> Task:
        return self._get_owned_task(user_id, task_id)
    ...
    def list_tasks_in_range(self, user_id: str, start_day: date, end_day: date) -> list[Task]:
        return self.task_repository.list_range(user_id, start_day, end_day)
    ...
    def mark_active(self, user_id: str, task_id: str) -> Task:
        task = self.get_task(user_id, task_id)
    ...
    def mark_done(self, user_id: str, task_id: str) -> Task:
    ...
    def mark_missed(self, user_id: str, task_id: str) -> Task:
        task = self.get_task(user_id, task_id)
    ...
    def add_chunk(self, user_id: str, task_id: str, payload: TaskChunkCreate) -> TaskChunk:
        task = self.get_task(user_id, task_id)
    ...
    def update_chunk(self, user_id: str, task_id: str, chunk_id: str, payload: TaskChunkUpdate) -> TaskChunk:
        task = self.get_task(user_id, task_id)
    ...
    def mark_chunk_done(self, user_id: str, task_id: str, chunk_id: str, actual_minutes: int | None = None) -> Task:
        self.get_task(user_id, task_id)
    ...
    def delete_chunk(self, user_id: str, task_id: str, chunk_id: str) -> None:
        task = self.get_task(user_id, task_id)
        chunk = self.chunk_repository.get(chunk_id)
        if not chunk or chunk.task_id != task.id:
            raise NotFoundError("Chunk not found")
