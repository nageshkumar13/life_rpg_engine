...
class BacklogService:
    ...
    def get_backlog(self, user_id: str, backlog_id: str) -> BacklogTask:
        backlog = self.backlog_repository.get(backlog_id)
        if not backlog or backlog.user_id != user_id:
            raise NotFoundError("Backlog item not found")
        return backlog

    def update_backlog(self, user_id: str, backlog_id: str, payload: BacklogUpdate) -> BacklogTask:
        backlog = self.get_backlog(user_id, backlog_id)
    ...
    def delete_backlog(self, user_id: str, backlog_id: str) -> None:
        backlog = self.get_backlog(user_id, backlog_id)
        self.backlog_repository.delete(backlog)
        self.db.commit()

    def assign_to_day(self, user_id: str, backlog_id: str, assigned_day: date) -> tuple[BacklogTask, object]:
        backlog = self.get_backlog(user_id, backlog_id)
